...
def test_postgres_ready():
    r = subprocess.run(["docker", "exec", "synapse_postgres", "pg_isready", "-U", "postgres"], capture_output=True, text=True)
...
def test_pgvector_extension_installed():
    r = subprocess.run(["docker", "exec", "synapse_pgvector", "psql", "-U", "postgres", "-d", "rag_db", "-c", "SELECT extname FROM pg_extension WHERE extname='vector';"], capture_output=True, text=True)
...
def test_mongodb_ping():
    r = subprocess.run(["docker", "exec", "synapse_mongodb", "mongosh", "--eval", "db.adminCommand({ping:1})"], capture_output=True, text=True)
...
def test_redis_ping():
    r = subprocess.run(["docker", "exec", "synapse_redis", "redis-cli", "PING"], capture_output=True, text=True)
...
def test_rabbitmq_running():
    r = subprocess.run(["docker", "exec", "synapse_rabbitmq", "rabbitmq-diagnostics", "status"], capture_output=True, text=True)
